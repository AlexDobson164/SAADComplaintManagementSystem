...
Chosen option: "PostgreSQL", because PostgrSQL better fits the requirements of this project. This is beacuase it can handle more data types than SQLite and the solution needs to be able to accomodate the 10% yearly increase in users. The support life for new versions of PostgrSQL is also longer than other technologies that I am using in this project as well as there being a guide on how to upgrade it.
...
An open source. object relational database. Allows for the creation of roles that only have specific privilages in the database, such as only being able to interact with specific tables
...
* Bad, because The SQLite site says its self that it should only bne used for small to medium websites.
